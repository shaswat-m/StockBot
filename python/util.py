...
class LSTM_Model:
    ...
    def custom_loss_def(self, y_true, y_pred):
        ...
        self.weights = np.float32(
            1.0 + 1.0 * np.linspace(0, self.forward_look - 1, self.forward_look) / 200.0
        )
        return tf.math.reduce_mean(
            tf.math.square(tf.multiply(self.weights, y_true - y_pred))
        )
    ...
    def data_preprocess(self, dataset, iStart, iEnd, sHistory, forward_look=1):
        ...
        for i in range(iStart, iEnd):
            ...
            reshape_entity = np.append(
                reshape_entity, dataset[indices]
            )  # Comment this out if there are multiple identifiers in the feature vector
    ...
    def prepare_test_train(self):
        ...
        training_std = self.y[
            :training_size
        ].std()  # std = a measure of how far away individual measurements tend to be from the mean value of a data set.
        self.y = (
            self.y - training_mean
        ) / training_std  # prep data, use mean and standard deviation to maintain distribution and ratios
        self.data_preprocess(
            self.y, 0, training_size, self.past_history, forward_look=self.forward_look
        )
        ...
        self.data_preprocess(
            ...
            None,
            self.past_history,
            forward_look=self.forward_look,
        )
        ...
        if self.naive:
            ...
            self.model.add(
                tf.keras.layers.LSTM(
                    20, return_sequences=True, input_shape=self.xtrain.shape[-2:]
                )
            )
    ...
    def infer_values(self, xtest, ytest, ts):
        ...
        for i in range(self.values):
            ...
            self.y_pred_update = self.model.predict(
                self.usetest[i, :, :].reshape(1, xtest.shape[1], xtest.shape[2])
            )[0][:]
            self.pred.append(self.y_pred)
            self.pred_update.append(self.y_pred_update)
            self.usetest[
                np.linspace(
                    i + 1, i + self.past_history - 1, self.past_history - 1, dtype=int
                ),
                np.linspace(self.past_history - 2, 0, self.past_history - 1, dtype=int),
                :,
            ] = self.y_pred_update[0]
            if self.infer_train:
                self.y_pred_train = self.model.predict(
                    self.xtrain[i, :, :].reshape(
                        1, self.xtrain.shape[1], self.xtrain.shape[2]
                    )
                )[0][:]
                self.y_pred_update_train = self.model.predict(
                    self.usetest_train[i, :, :].reshape(
                        1, self.xtrain.shape[1], self.xtrain.shape[2]
                    )
                )[0][:]
                ...
                self.usetest_train[
                    np.linspace(
                        i + 1,
                        i + self.past_history - 1,
                        self.past_history - 1,
                        dtype=int,
                    ),
                    np.linspace(
                        self.past_history - 2, 0, self.past_history - 1, dtype=int
                    ),
                    :,
                ] = self.y_pred_update_train[0]
        ...
        if self.forward_look > 1:
            self.RMS_error_update = (
                np.mean(
                    (
                        (self.ytest[: self.values - 1, 0, 0] - self.pred_update[1:, 0])
                        / (self.ytest[: self.values - 1, 0, 0])
                    )
                    ** 2
                )
            ) ** 0.5 / self.batch_size
            if self.infer_train:
                self.RMS_error_update_train = (
                    np.mean(
                        (
                            (
                                self.ytrain[: self.values - 1, 0, 0]
                                - self.pred_update_train[1:, 0]
                            )
                            / (self.ytrain[: self.values - 1, 0, 0])
                        )
                        ** 2
            ...
            if self.infer_train:
                self.RMS_error_update_train = (
                    np.mean(
                        (
                            (
                                self.ytrain[: self.values - 1]
                                - self.pred_update_train[1:]
                            )
                            / (self.ytrain[: self.values - 1])
                        )
                        ** 2
    ...
    def plot_test_values(self, suffix=""):
        ...
        if self.forward_look > 1:
            ...
            plt.figure()
            plt.plot(
                self.pred[1:, 0] - self.pred_update[1:, 0],
                label="difference (%s)" % self.ts,
            )
            plt.xlabel("Days")
            ...
            plt.figure()
            plt.plot(
                self.pred[1:] - self.pred_update[1:], label="difference (%s)" % self.ts
            )
            plt.xlabel("Days")
            plt.ylabel("Prediction difference")
            plt.savefig(
        ...
        if self.infer_train:
            ...
            print(
                "The relative train RMS error for the updated dataset is %f"
                % self.RMS_error_update_train
            )
    ...
    def arch_plot(self):
        ...
        """
        dot_img_file = "../images/LSTM_arch_depth%d_naive%d.png" % (
            self.depth,
            int(self.naive),
        )
    ...
    def plot_bot_decision(self, suffix=""):
        ...
        if self.forward_look > 1:
            ...
            pred = np.asarray(self.pred[1:, 0]).reshape(
                -1,
            )
            pred_update = np.asarray(self.pred_update[1:, 0]).reshape(
                -1,
            )
        else:
            ...
            pred_update = np.asarray(self.pred_update[1:]).reshape(
                -1,
            )
...
class LSTM_ED_Model:
    ...
    def data_preprocess(self, dataset, iStart, iEnd, sHistory, forward_look=1):
        ...
        for i in range(iStart, iEnd):
            ...
            reshape_entity_x = np.append(
                reshape_entity_x, dataset[indices_x]
            )  # Comment this out if there are multiple identifiers in the feature vector
            reshape_entity_x_dec = np.asarray([])
            reshape_entity_x_dec = np.append(
                reshape_entity_x_dec, dataset[indices_x_dec]
            )  # Comment this out if there are multiple identifiers in the feature vector
            reshape_entity_y_dec = np.asarray([])
            reshape_entity_y_dec = np.append(
                reshape_entity_y_dec, dataset[indices_y_dec]
            )  # Comment this out if there are multiple identifiers in the feature vector
    ...
    def prepare_test_train(self):
        ...
        self.xtrain, self.xtrain_dec, self.ytrain = (
            self.data_enc,
            self.data_dec,
            self.target,
        )
        ...
        self.xtest, self.xtest_dec, self.ytest = (
            self.data_enc,
            self.data_dec,
            self.target,
        )
    ...
    def create_p_test_train(self):
        ...
        p_train = tf.data.Dataset.from_tensor_slices(
            ((self.xtrain, self.xtrain_dec), self.ytrain)
        )
        self.p_train = (
            p_train.cache()
        ...
        p_test = tf.data.Dataset.from_tensor_slices(
            ((self.xtest, self.xtest_dec), self.ytest)
        )
        self.p_test = p_test.batch(BATCH_SIZE).repeat()
    ...
    def model_LSTM(self):
        ...
        )  # Number of latent dimensions, defaults to 20
    ...
    def infer_values(self, xtest, ytest, ts):
        ...
        decoder_input = xtest[
            0:1, -1, :
        ]  # choosing the most recent value to feed the decoder
        for i in range(self.values):
    ...
    def plot_test_values(self):
        ...
        if self.forward_look > 1:
            ...
            plt.plot(
                self.ytest[: self.values, 0, 0], "-", label="actual (%s)" % self.ts
            )
            ...
            plt.legend()
            plt.savefig(
                "../images/ED_Stock_prediction_%d_%d_%d_%d_%s.png"
                % (
                    self.depth,
                    int(self.naive),
                    self.past_history,
    ...
    def arch_plot(self):
        ...
        Plot the network architecture
        """
        dot_img_file = "../images/LSTM_ED_arch_depth%d_naive%d.png" % (
            self.depth,
            int(self.naive),
        )
        tf.keras.utils.plot_model(self.model, to_file=dot_img_file, show_shapes=True)
    ...
    def plot_bot_decision(self):
        ...
        if self.forward_look > 1:
            ...
            pred = np.asarray(self.pred[0, 1:, 0]).reshape(
                -1,
            )
        else:
            ideal = self.ytest[: self.values - 1, 0, 0]
        ...
        plt.legend()
        plt.savefig(
            "../images/ED_Bot_prediction_%d_%d_%d_%d.png"
            % (self.depth, int(self.naive), self.past_history, self.forward_look)
        )
        np.savez(
            "../save_mat/EDbot_%d_%d_%d_%d_%s_%s.png"
            % (
                self.depth,
                int(self.naive),
                self.past_history,
...
class LSTM_Model_MS:
    ...
    def get_ticker_values(self):
        ...
        for tickerSymbol in self.tickerSymbolList:
            ...
            self.maxTestValues = len(data.values) - int(
                len(data.values) * self.train_test_split
            )
        if (
            self.sameTickerTestTrain == False
        ):  # This indicates self.tickerSymbol is the test ticker and self.tickerSymbolList is the training set
            tickerData = yf.Ticker(self.tickerSymbol)
            tickerDf = yf.download(self.tickerSymbol, start=self.start, end=self.end)
            tickerDf = tickerDf["Close"]
            data = tickerDf
    ...
    def prepare_test_train(self):
        ...
        self.y_size = 0
        if (
            self.sameTickerTestTrain == True
        ):  # For each ticker, split data into train and test set. Test and validation are the same
            self.xtrain = []
            self.ytrain = []
            self.xtest = []
            self.ytest = []
            for y in self.y_all:
                ...
                data, target = self.data_preprocess(
                    y,
                    0,
                    training_size,
                    self.past_history,
            ...
            data, target = self.data_preprocess(
                y, 0, validation_size, self.past_history, forward_look=self.forward_look
            )
            self.xtest = data
            self.ytest = target
            data, target = self.data_preprocess(
    ...
    def plot_test_values(self):
        ...
        if self.forward_look > 1:
            ...
            plt.legend()
            plt.savefig(
                "../images/MultiStock_prediction_%d_%d_%d_%d_%s.png"
                % (
                    self.depth,
                    int(self.naive),
                    self.past_history,
            ...
            plt.ylabel("Prediction difference")
            plt.savefig(
                "../images/MSDifference_%d_%d_%d_%d_%s.png"
                % (
                    self.depth,
                    int(self.naive),
                    self.past_history,
    ...
    def prepare_test(self):
        ...
        training_std = self.ytemp[
            :training_size
        ].std()  # std = a measure of how far away individual measurements tend to be from the mean value of a data set.
        self.ytemp = (
            self.ytemp - training_mean
        ) / training_std  # prep data, use mean and standard deviation to maintain distribution and ratios
        data, target = self.data_preprocess(
            self.ytemp,
            training_size,
    ...
    def plot_bot_decision(self):
        ...
        plt.legend()
        plt.savefig(
            "../images/MSBot_prediction_%d_%d_%d_%d_%s.png"
            % (
                self.depth,
                int(self.naive),
                self.past_history,
        ...
        np.savez(
            "../save_mat/MSbot_%d_%d_%d_%d_%s_%s.png"
            % (
                self.depth,
                int(self.naive),
                self.past_history,
...
class LSTM_Model_MS_GT:
    ...
    def get_gtrends_data(self, tickerName):
        ...
        for i in range(0, 11):
            ...
            t.set_active_gtab(
                "google_anchorbank_geo=US_timeframe=" + timeframe_str + ".tsv"
            )
    ...
    def get_ticker_values(self):
        ...
        for tickerSymbol in self.tickerSymbolList:
            ...
            tset = np.concatenate(
                [data.reshape([-1, 1]), trend_points.values.reshape([-1, 1])], axis=1
            )
        ...
        if (
            ...
            for y in self.y_all:
                ...
                training_std = y[:training_size].std(
                    axis=0
                )  # std = a measure of how far away individual measurements tend to be from the mean value of a data set.
                y[:, 0] = (y[:, 0] - training_mean[0]) / training_std[
                    0
                ]  # prep data, use mean and standard deviation to maintain distribution and ratios
                data, target = self.data_preprocess(
                    y,
                    0,
                    training_size,
            ...
            validation_std = y[:validation_size].std(
                axis=0
            )  # std = a measure of how far away individual measurements tend to be from the mean value of a data set.
    ...
    def plot_test_values(self):
        ...
        if self.forward_look > 1:
            ...
            plt.legend()
            plt.savefig(
                "../images/GTMultiStock_prediction_%d_%d_%d_%d_%s.png"
                % (
                    self.depth,
                    int(self.naive),
                    self.past_history,
            ...
            plt.ylabel("Prediction difference")
            plt.savefig(
                "../images/GTMSDifference_%d_%d_%d_%d_%s.png"
                % (
                    self.depth,
                    int(self.naive),
                    self.past_history,
    ...
    def plot_bot_decision(self):
        ...
        plt.legend()
        plt.savefig(
            "../images/GTMSBot_prediction_%d_%d_%d_%d_%s.png"
            % (
                self.depth,
                int(self.naive),
                self.past_history,
        ...
        np.savez(
            "../save_mat/GTMSbot_%d_%d_%d_%d_%s_%s.png"
            % (
                self.depth,
                int(self.naive),
                self.past_history,
